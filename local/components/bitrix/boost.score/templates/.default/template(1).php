...
$strSql = "Select ID,NAME,SECOND_NAME,LAST_NAME,EMAIL,c.UF_DEPARTMENT AS DEP from b_user a,b_user_group b,b_uts_user c where a.active ='Y' and a.LID ='s1' and a.ID = b.USER_ID and b.GROUP_ID='11' and c.VALUE_ID=a.ID and a.ID NOT IN (1,27,1709,1013,4817,4818)  ORDER BY a.ID ASC";
...
$leadconvSqltot = $DB->Query("select count(*) as lead_convertion_count from b_user a,b_user_group b,b_uts_user c,b_crm_lead d where a.active ='Y' and a.LID ='s1' and a.ID = b.USER_ID and b.GROUP_ID='11' and c.VALUE_ID=a.ID and a.ID NOT IN (1,27,1709,1013,4817,4818) and a.ID=d.ASSIGNED_BY_ID and d.STATUS_ID = 'CONVERTED' and d.DATE_MODIFY between '".$fromdate."' and '".$todate."'  ");
...
$strtimeSqltot = $DB->Query("Select sum(TIMESTAMPDIFF(SECOND,DATE_START,DATE_FINISH)) AS score_time from b_user a,b_user_group b,b_uts_user c,b_timeman_entries d where a.active ='Y' and a.LID ='s1' and a.ID = b.USER_ID and b.GROUP_ID='11' and c.VALUE_ID=a.ID and a.ID NOT IN (1,27,1709,1013,4817,4818) and a.ID=d.USER_ID and d.DATE_START between '".$fromdate."' and '".$todate."' and d.DATE_FINISH between '".$fromdate."' and '".$todate."'");
...
$stractSqltot = $DB->Query("Select count(*) as activities_count from b_user a,b_user_group b,b_uts_user c,b_crm_act d where a.active ='Y' and a.LID ='s1' and a.ID = b.USER_ID and b.GROUP_ID='11' and c.VALUE_ID=a.ID and a.ID NOT IN (1,27,1709,1013,4817,4818) and a.ID=d.RESPONSIBLE_ID and d.OWNER_TYPE_ID ='1' and d.CREATED between '".$fromdate."' and '".$todate."'");
$actRestot = $stractSqltot->Fetch();


$stractper = round($actRes['activities_count'] / $actRestot['activities_count'] * 25, 2)."%\n";
...
$strproactSqltot = $DB->Query("Select count(*) as proactivities_count from b_user a,b_user_group b,b_uts_user c,c_distribution_lead_missing d where a.active ='Y' and a.LID ='s1' and a.ID = b.USER_ID and b.GROUP_ID='11' and c.VALUE_ID=a.ID and a.ID NOT IN (1,27,1709,1013,4817,4818) and a.ID=d.USER_ID and d.MISSED_AT between '".$fromdate."' and '".$todate."' ");
$proactRestot = $strproactSqltot->Fetch();

if($proactRes['proactivities_count'] > 0)
{
$proactper = round($proactRes['proactivities_count'] / $proactRestot['proactivities_count'] * 25, 2)."%\n";
}
/*------------------------Active Leads------------------------------*/
...
<td align='left' style='border:1px #f5f9f9 solid;width:15%;'><a href="/company/personal/user/<?=$arRes['ID'];?>/" bx-tooltip-user-id="<?=$arRes['ID'];?>"><?=$arRes['NAME']." ".$arRes['LAST_NAME']; ?></a></td>
	<td align='center' style='border:1px #f5f9f9 solid;width:5%;'><a href="/local/custom-reports/agent-score/time-details/?USER_ID=<?=$arRes['ID'];?>"><?php  echo "{$timeduration['hours']}h". " {$timeduration['minutes']}m"; ?></a></td>	
...
<p style="color:red;">
    ...
    1. Each Employee Individual Percentage (100%) =  Lead Conversion (40%) + Attendance (10%) + Activities (25%) - Reactiveness (25%) <br/>
