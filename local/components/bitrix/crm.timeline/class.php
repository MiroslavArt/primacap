...
class CCrmTimelineComponent extends CBitrixComponent
{
	...
	{
        $scheduledItems = $this->getRepository()->getScheduledItems()->getItems();
		$rstrictscheduledItems = $this->getRepository()->getScheduledItems((new Crm\Service\Timeline\Repository\Query())
        ->setFilter( ['AUTHOR_ID' => $this->arResult['USER_ID']]))->getItems();
...
$resp= $DB->query("SELECT ASSIGNED_BY_ID FROM b_crm_event_relations where ENTITY_FIELD = 'ASSIGNED_BY_ID' AND ENTITY_ID='". $this->entityID."' ORDER BY ID DESC LIMIT 0, 1");
$resp_res = $resp->Fetch();
$assigned_by_id = $resp_res['ASSIGNED_BY_ID'];
		/*if($USER->IsAdmin() || in_array('18',$arUser['UF_DEPARTMENT']) || $USER_ID=='4647' || $USER_ID=='4481'){
   $this->arResult['SCHEDULE_ITEMS'] = $scheduledItems;
} else {
 $this->arResult['SCHEDULE_ITEMS'] = $rstrictscheduledItems;

}*/
if( in_array('22',$arUser['UF_DEPARTMENT']) && $assigned_by_id=='4647'){
 $this->arResult['SCHEDULE_ITEMS'] = $rstrictscheduledItems;
} else {
$this->arResult['SCHEDULE_ITEMS'] = $scheduledItems;
	...
	public function prepareHistoryItems($offsetTime = null, $offsetID = 0)
	{
		...
		$result_rstrict = $this->getRepository()->getHistoryItemsPage(
			(new Crm\Service\Timeline\Repository\Query())
				->setOffsetId((int)$offsetID)
			     ->setOffsetTime($offsetTime ? DateTime::createFromUserTime($offsetTime) : null)
                ->setFilter(array_merge($filter, ['AUTHOR_ID' => $this->arResult['USER_ID']]))
			    ->setLimit(100000)
		);
global $DB,$USER;
$USER_ID = $USER->GetID();
...
$assigned_by_id = $resp_res['ASSIGNED_BY_ID'];
		...
		$this->arResult['HISTORY_ITEMS'] = $result_rstrict->getItems();
}*/
